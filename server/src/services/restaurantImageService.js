...
const ENDPOINT = 'POST /api/v1/restaurants/:restaurantId/images';
...
async function getIdempotency(client, userId, idempotencyKey) {
...
async function createIdempotency(client, userId, idempotencyKey, hash) {
  const result = await client.query(
...
async function refreshIdempotency(client, userId, idempotencyKey, hash) {
  const result = await client.query(
...
async function markIdempotencyFailed(userId, idempotencyKey, hash, leaseUntil) {
  let client;
  try {
...
async function prepareUpload({
  ...
  try {
    ...
    const existing = await getIdempotency(client, userId, idempotencyKey);
    ...
    if (resolution?.retryable) {
      leaseUntil = await refreshIdempotency(client, userId, idempotencyKey, hash);
    } else {
      leaseUntil = await createIdempotency(client, userId, idempotencyKey, hash);
    }
    await client.query('COMMIT');
    return { leaseUntil };
  } catch (err) {
...
export async function uploadRestaurantImage({
  ...
  try {
    ...
    await markIdempotencyFailed(userId, idempotencyKey, hash, leaseUntil);
    throw err;
  }

  try {
    const body = await persistImage({
      userId,
      roles,
      restaurantId,
      idempotencyKey,
