...
export const billScanUpload = multer({
  storage: multer.memoryStorage(),
  limits: {
    fileSize: RECEIPT_MAX_BYTES,
    files: 1,
...
export function validateBillScanUploadMetadata(req, res, next) {
  const idempotencyKey = req.headers['idempotency-key'];
  if (!idempotencyKey) {
    next(createHttpError(400, 'IDEMPOTENCY_KEY_REQUIRED', 'Idempotency-Key header is required.'));
    return;
...
export function uploadSingleBillScanImage(req, res, next) {
  billScanUpload.single('receiptImage')(req, res, (err) => {
    if (!err) {
      next();
      return;
    }
