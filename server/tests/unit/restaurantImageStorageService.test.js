...
const originalRestaurantBucket = process.env.AWS_RESTAURANT_IMAGES_BUCKET_NAME;
const originalDefaultBucket = process.env.AWS_S3_BUCKET_NAME;
...
async function loadService({
  ...
  if (bucketName == null) {
    delete process.env.AWS_RESTAURANT_IMAGES_BUCKET_NAME;
    delete process.env.AWS_S3_BUCKET_NAME;
  } else {
    process.env.AWS_RESTAURANT_IMAGES_BUCKET_NAME = bucketName;
  }
...
describe('S3 restaurant image storage service', () => {
  afterEach(() => {
    if (originalRestaurantBucket === undefined) delete process.env.AWS_RESTAURANT_IMAGES_BUCKET_NAME;
    else process.env.AWS_RESTAURANT_IMAGES_BUCKET_NAME = originalRestaurantBucket;
    if (originalDefaultBucket === undefined) delete process.env.AWS_S3_BUCKET_NAME;
    else process.env.AWS_S3_BUCKET_NAME = originalDefaultBucket;
    vi.restoreAllMocks();
    vi.resetModules();
  });
  ...
  it('fails closed when provider configuration is incomplete', async () => {
    ...
    await expect(service.deleteRestaurantImageReference(
      'https://legacy-cdn.test/restaurant.jpg',
    )).resolves.toBe(false);
  });
});
