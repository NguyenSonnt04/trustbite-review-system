...
`npm run harness -- query tools`. It lets agents discover available
...
The advisory result from `npm run harness -- score-context <trace-id>`.
It compares a trace's recorded `files_read` against compiled context rules and
retrieval triggers.

## Entropy Score

The drift score printed by `npm run harness -- audit`. Lower is better.
...
A structured recommendation generated by `npm run harness -- propose` from
...
The SQLite database and CLI (`npm run harness -- ...`) that stores operational records
