...
| Harness improvement | Improving how humans and agents collaborate | Direct docs update or `npm run harness -- backlog add` |
...
- Record or update proof status with `npm run harness -- story add` and
...
- Record a durable decision when behavior, architecture, authorization, data
  ...
  or refresh the durable row with `npm run harness -- decision add`.
