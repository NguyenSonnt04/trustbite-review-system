...
| Layer | Cases |
| --- | --- |
| Unit | Status transition validation, self-suspend guard, self-reactivate guard, deleted-user suspend/reactivation guards, admin tier check uses normalized `user_roles`, suspend validation order (`DELETED` before self before already-`SUSPENDED` before reason), and reactivate validation order (`DELETED` before self before not-`SUSPENDED` before reason). |
