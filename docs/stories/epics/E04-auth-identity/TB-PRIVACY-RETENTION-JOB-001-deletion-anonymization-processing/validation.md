...
- Extended receipt/review proof minimization: deleted-user reviews now use neutral ratings, `verification_status = 'DELETED'`, and `visited_at = NULL`; deleted-user receipt verifications now receive deterministic tombstone `file_hash_sha256`, retained fraud-minimum `transaction_unique_hash`, `status = 'DELETED'`, `fraud_risk_score = 0`, `decision = NULL`, `decision_reason = NULL`, and `decided_by = NULL`; cross-record `receipt_verifications.decided_by` and `restaurant_claims.decided_by` references to the deleted user are cleared without tombstoning other users' receipt/claim content.
...
2026-07-07 review follow-up for retry starvation and receipt transaction replay:

- Added migration `005_retain_deleted_receipt_transaction_hash.sql` so `idx_receipts_trx_uniq` also covers `DELETED` receipt rows with retained transaction hashes.
- Kept `receipt_verifications.transaction_unique_hash` as fraud-minimum evidence only for previously verified receipts during account deletion, preventing a deleted verified receipt transaction from being replayed by another account while clearing non-verified transaction hashes.
- Capped automatic cleanup selection at 5 attempts so permanent `PROCESSING`/`RETRYABLE` failures remain fail-closed without starving later due requests in the batch.

2026-07-07 Greptile 4/5 closeout follow-up for exhausted retries and S3 cleanup duration:

- Added migration `006_add_failed_account_deletion_cleanup_state.sql` so exhausted automatic deletion cleanup retries can be marked with terminal request-level `cleanup_state = FAILED` without inventing a new deletion-request status.
- Added integration regression proof that a `PROCESSING`/`RETRYABLE` request at 5 cleanup attempts is marked `FAILED` with `cleanup_last_error_code = CLEANUP_ATTEMPTS_EXHAUSTED`, retained-data reason `account deletion cleanup attempts exhausted; operator review required`, and non-sensitive `ACCOUNT_DELETION_CLEANUP_EXHAUSTED` audit evidence, while a later due request can still complete when the batch has remaining budget.
- Changed owned-object cleanup to issue S3 delete calls concurrently after provider cleanup, reducing advisory-lock hold time for users with multiple owned objects while keeping cleanup fail-closed through an all-settled wait-then-throw path.
- Added integration proof that avatar plus receipt object cleanup calls overlap (`maxInFlightDeletes > 1`) and still complete the deletion request, plus a partial-failure regression proving every started object delete settles before the request is marked retryable and the advisory lock is released.
- Added regression proof that legal-held retryable rows at the attempt cap still take the legal-hold path (`cleanup_state = LEGAL_HOLD`) instead of being marked exhausted, and that exhausted markers are ordered and counted against the configured batch size.
- Updated deletion job summary logging so CLI output includes the aggregate `exhausted` counter without exposing request ids or user ids.
- Red/green proof: the targeted integration first failed with the maxed row still `RETRYABLE`, `maxInFlightDeletes = 1`, the partial-failure path returning before all object deletes settled, the maxed legal-hold row hidden by exhausted retry handling, and exhausted marker work exceeding the batch budget; after the processor and migration changes, `npm run test:integration --prefix server -- tests/integration/accountDeletionProcessor.integration.test.js` passed with 5 integration files / 43 tests and 1 skipped file / 2 skipped tests.
- Final validation passed: `npm run server:test` with 16 server test files / 141 tests and 1 skipped file / 2 skipped tests; `npm run server:build` with 85-file syntax proof; `npm run db:migrate` applied 0 migrations; `npm run harness -- story verify TB-PRIVACY-RETENTION-JOB-001` passed with 16 server test files / 141 tests and 85-file syntax proof.
