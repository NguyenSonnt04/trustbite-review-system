...
- Extended review-derived cleanup: detaches `price_history.review_id` for deleted-user reviews, deletes stale restaurant-level `review_summaries`, and recomputes affected `restaurants.trust_score`, `verified_review_count`, and `reference_review_count` from remaining public trusted reviews, with neutral `5.00` when no eligible reviews remain.
