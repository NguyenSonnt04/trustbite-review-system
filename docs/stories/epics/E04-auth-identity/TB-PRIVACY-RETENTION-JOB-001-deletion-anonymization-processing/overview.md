...
- No arbitrary schema changes beyond the accepted cleanup-state migrations for retry, lease, legal-hold, and exhausted-retry proof. If safe completion needs provider-key fields, per-object cleanup rows, per-category retention expiry, or broader ownership markers, pause for a high-risk migration/decision update.
