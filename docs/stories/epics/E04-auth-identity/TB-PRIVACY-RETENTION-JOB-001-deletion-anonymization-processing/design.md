...
- Processing claim: the mechanism that prevents duplicate workers from completing the same request. With the current schema, the only acceptable full-cleanup strategy is an explicit retryable claim design: claim due `REQUESTED` rows into `PROCESSING`, commit a local fail-closed guard, keep cleanup handles durable until completion, retry incomplete `PROCESSING` rows on later runs, and protect each attempt with a session-level PostgreSQL advisory lock or an equivalent documented concurrency guard that spans non-transactional provider/object calls without holding a long DB transaction open. There is no `processing_started_at`, `failed_at`, `error_code`, retry counter, `FAILED`, `LEGAL_HOLD`, or `ANONYMIZED` deletion-request status yet, so any implementation that needs leases, backoff, per-provider status, per-object status, or legal hold markers must add the migration/decision before provider/object cleanup is implemented. Provider and object-storage cleanup are non-transactional side effects; do not hold a long DB transaction around remote calls unless the calls are bounded, idempotent, retry-safe, and covered by explicit proof.
...
3. Process claimed `PROCESSING` rows idempotently. Because the schema has no lease columns, every run must find `PROCESSING` rows that are not `COMPLETED`, take a session-level advisory lock before provider/storage cleanup, and retry cleanup from the durable handles. Derive the lock key in application code as a deterministic signed 64-bit integer from the first 8 bytes of `sha256('trustbite.account_deletion_request:' || deletion_request_id::text)`, then call `pg_advisory_lock(bigint)` and release it with `pg_advisory_unlock(bigint)` in a `finally` path; PostgreSQL releases the session lock if the connection dies. Do not use transaction-scoped advisory locks for provider/storage calls, because they either require a long transaction around remote calls or release before the duplicate-work window is protected. If the hash-collision risk, starvation/backoff, stuck-row alerting, per-provider status, or retry limits are unacceptable, stop for a migration/decision that adds a cleanup-work/lease schema instead of adding in-memory-only recovery.
