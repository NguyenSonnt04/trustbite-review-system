...
- Processing claim: the mechanism that prevents duplicate workers from completing the same request. With the current schema, the only acceptable full-cleanup strategy is an explicit retryable claim design: claim due `REQUESTED` rows into `PROCESSING`, commit a local fail-closed guard, keep cleanup handles durable until completion, retry incomplete `PROCESSING` rows on later runs, and protect each attempt with a PostgreSQL advisory lock or an equivalent documented concurrency guard. There is no `processing_started_at`, `failed_at`, `error_code`, retry counter, `FAILED`, `LEGAL_HOLD`, or `ANONYMIZED` deletion-request status yet, so any implementation that needs leases, backoff, per-provider status, per-object status, or legal hold markers must add the migration/decision before provider/object cleanup is implemented. Provider and object-storage cleanup are non-transactional side effects; do not hold a long DB transaction around remote calls unless the calls are bounded, idempotent, retry-safe, and covered by explicit proof.
...
3. Process claimed `PROCESSING` rows idempotently. Because the schema has no lease columns, every run must find `PROCESSING` rows that are not `COMPLETED`, take a transaction-scoped advisory lock keyed by the deletion request id (or an equivalent documented per-request lock), and retry cleanup from the durable handles. If starvation/backoff, stuck-row alerting, per-provider status, or retry limits are required, stop for a migration/decision rather than adding in-memory-only recovery.
...
5. Revoke remaining local `user_sessions` by setting `revoked_at = COALESCE(revoked_at, now())`. Treat these rows as local product/device state, not Cognito refresh-token ownership. Because request-time code may already have revoked every active row, processing must prove idempotent no-op behavior for already-revoked sessions. Because `refresh_token_hash` is `NOT NULL`, any retained session rows must either be deleted where allowed or overwritten with per-row non-sensitive tombstones; `device_label` should be cleared/minimized. Because Cognito-issued access tokens can remain valid until expiry, completion proof must show auth rejects the user via local `DELETED` state or unmapped identity even when the token verifies.
6. Deactivate remaining `push_tokens` by moving active tokens to `INACTIVE`; do not log `token_ciphertext` or full fingerprints. Status-only deactivation is not enough for deletion completion because `token_ciphertext` and `token_fingerprint` remain sensitive and `NOT NULL`; rows must be deleted where allowed or overwritten with per-row schema-valid tombstones that preserve the `(user_id, token_fingerprint)` uniqueness constraint. Prove this is idempotent when request-time revocation already moved all active tokens to `INACTIVE`.
...
9. Anonymize or clear profile/identity PII on `users`:
   ...
   - after provider cleanup and phone-keyed OTP cleanup no longer need the original value, replace `phone_number` with a deterministic non-contactable tombstone value that preserves uniqueness and satisfies `VARCHAR(20) UNIQUE NOT NULL`,
   ...
   - reset local profile/gamification surfaces stored on the user row, such as `exp_points = 0`, `rank_code = 'NEWBIE'` when that seeded rank exists, and `review_restricted_until = NULL` unless a fraud/legal retention rule explicitly requires keeping the restriction timestamp,
   - set `status = DELETED`,
   - set `deleted_at = now()`,
...
10. Apply the schema-backed retention map for related records. Do not invent columns outside migrations:
    ...
    - Restaurant review aggregates and summaries: update, invalidate, or queue recomputation for `restaurants.trust_score`, `verified_review_count`, `reference_review_count`, `review_summaries.summary_text`, `review_summaries.review_count`, and `review_summaries.avg_rating` that could include deleted-user review content. The migrated `review_summaries` table has no status/invalidation column, so if no recomputation worker/queue exists by implementation time, delete affected `review_summaries` rows rather than inventing an invalidation field or leaving stale generated text. Set aggregate counters from the retained non-deleted review set inside the same proof path; when there are zero retained eligible reviews, set `verified_review_count = 0`, `reference_review_count = 0`, and either reset `restaurants.trust_score` to the schema/product default `5.00` with documented neutral-score semantics or stop for the trust-score dependency if that neutral rule is not accepted. `restaurants.trust_score` must otherwise be recomputed with the accepted backend trust-score rule or, if that rule/calculator still does not exist, the implementation must stop for the trust-score dependency instead of leaving a stale score that still reflects deleted reviews. Do not leave summaries, aggregate counters, or trust scores treating `DELETED` reviews as public/trustable.
    ...
    - Receipt line items and menu maps: delete line items when allowed so `receipt_line_item_menu_maps` cascades, or anonymize purchase details through schema-valid operations. `raw_item_name`, `quantity`, `unit_price`, and `total_price` can expose receipt contents; because `raw_item_name`, `quantity`, `unit_price`, and `total_price` are `NOT NULL`, use deletion, retained-with-reason, or non-PII/non-sensitive per-row tombstones if retained.
...
The current migrated schema lacks provider username, cleanup status, object cleanup status, legal hold, failure/retry, and processing lease columns. This story therefore either uses the retryable `PROCESSING` + local `DELETED` guard described above with durable cleanup handles kept in existing rows, or it must pause for a migration/decision before claiming cleanup completion.
