...
3. Process claimed `PROCESSING` rows idempotently. Because the schema has no lease columns, every run must find `PROCESSING` rows that are not `COMPLETED`, attempt a session-level advisory lock before provider/storage cleanup, and retry cleanup from the durable handles. Derive the lock key in application code as a deterministic signed 64-bit integer from the first 8 bytes of `sha256('trustbite-account-deletion-request:' || deletion_request_id::text)`: in Node.js, convert the hex digest with `Buffer.from(sha256Hex, 'hex').subarray(0, 8).readBigInt64BE(0)` so byte order is big-endian and the sign bit is handled exactly as PostgreSQL `bigint` expects. Call `pg_try_advisory_lock(bigint)` for the row; if it returns `false`, skip that row, increment `skipped`, and continue the batch instead of blocking the worker. Only after the try-lock returns `true` should the worker perform provider/storage cleanup for that request, and it must release the lock with `pg_advisory_unlock(bigint)` in a `finally` path; PostgreSQL releases the session lock if the connection dies. Do not use blocking `pg_advisory_lock` for processor retries unless a later decision documents the wall-clock timeout and fairness tradeoff. Do not use transaction-scoped advisory locks for provider/storage calls, because they either require a long transaction around remote calls or release before the duplicate-work window is protected. If the hash-collision risk, starvation/backoff, stuck-row alerting, per-provider status, or retry limits are unacceptable, stop for a migration/decision that adds a cleanup-work/lease schema instead of adding in-memory-only recovery.
