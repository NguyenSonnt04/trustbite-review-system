...
- A concrete recovery/idempotency rule for already-`PROCESSING` rows: claim into `PROCESSING` with a committed local `DELETED` fail-closed guard, retry `PROCESSING` rows using a per-request session-level `pg_try_advisory_lock` held on the same dedicated pool client for the full provider/storage cleanup attempt, persist request-level cleanup state/lease/retry/legal-hold fields, skip lock-busy rows instead of blocking the batch, and pause for a high-risk migration/decision if per-provider or per-object cleanup status is required.
...
- Cognito/provider account cleanup for the mapped identity where `users.cognito_sub` is present and configuration supports it, keeping all provider SDK calls behind `server/src/services/` or `server/src/config/`. Rows with `users.cognito_sub IS NULL` have no mapped Cognito account to clean up and may skip provider cleanup. Before implementation, prove whether a non-null `users.cognito_sub` is sufficient for the Cognito admin API or stop for a provider/schema decision that stores the needed username/handle. If provider cleanup for a mapped identity cannot be proven locally, document the exact blocker and do not claim provider-side deletion complete or request completion; local `DELETED`/unmapped-identity rejection still needs integration proof with otherwise-valid Cognito-compatible tokens.
...
- Open-deletion mutation guard ownership and proof: this story owns the Express auth/API guard that rejects protected mutations for users with an open `REQUESTED` or `PROCESSING` deletion request, unless a prior merged `TB-PRIVACY-DELETION-001` change explicitly adds the same hard gate and validation evidence. Without that proof, this story cannot be claimed complete.
...
10. Run or document provider cleanup proof for Cognito and S3/profile-avatar/receipt/media/claim-evidence objects. For this story, LocalStack S3 smoke proves object cleanup and a Cognito-compatible admin client test double proves Cognito sign-out/delete/missing-user behavior through the real provider boundary because LocalStack community 4.4.0 does not expose Cognito IdP admin APIs. Decision `0015-account-deletion-retention-scope` defers restaurant images, broader restaurant/menu/branch content, per-provider/per-object status tables, and per-category expiry to future high-risk stories.
...
- The schema lacks a required marker for retained legal/audit/fraud data beyond request-level `retained_data_reason`/`legal_hold_reason`, provider username/handle storage, per-provider cleanup status, per-object cleanup status, or per-category retained-data expiry.
