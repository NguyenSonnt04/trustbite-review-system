...
- Automatic cleanup retries are capped at the request level. When a `PROCESSING`/`RETRYABLE` request exhausts the automatic attempt cap, the processor marks request-level `cleanup_state = 'FAILED'` within the configured batch budget, records `CLEANUP_ATTEMPTS_EXHAUSTED` and non-sensitive audit evidence, keeps the account fail-closed, and leaves operator remediation or a future cleanup-work schema to a separate high-risk story.
- When present, `users.cognito_sub` is the Cognito admin cleanup identifier for the current Cognito-first boundary and is retained on the deleted local tombstone row as a security deny mapping. If it is `NULL`, the row has no mapped Cognito provider identity to clean up; the processor records provider cleanup as skipped instead of failing the deletion forever.
...
- Exhausted automatic retries are visible as request-level `FAILED` cleanup state, but the deletion request remains `PROCESSING` and the user remains `DELETED` until an operator or future remediation path resolves the failure.
