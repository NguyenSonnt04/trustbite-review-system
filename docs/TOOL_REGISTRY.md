...
```bash
npm run harness -- tool register --name gitnexus --kind mcp \
...
npm run harness -- tool register --name c3 --kind skill \
...
```bash
npm run harness -- tool remove --name deploy-check
```
...
```bash
npm run harness -- tool check            # scan all registered tools
npm run harness -- tool check --name c3  # scan one
npm run harness -- tool check --json     # machine-readable for agents
```
...
```bash
npm run harness -- query tools --capability impact-analysis
...
npm run harness -- query tools --json
npm run harness -- query tools --responsibility Verification
```
