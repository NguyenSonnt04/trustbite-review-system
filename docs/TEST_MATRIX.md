...
| TB-INFRA-001 | Real AWS MVP infrastructure baseline provisions non-production RDS/PostgreSQL+PostGIS, private S3 receipts, Redis/BullMQ cache, ECR, ECS API/worker runtime, IAM/secrets, CloudWatch, deploy workflow, smoke proof, and rollback notes | no | no | no | yes | in_progress | 2026-07-09 review follow-up: `create_ecs=true` now runs a reviewable ECS Terraform plan even when `apply=false`; ECS apply, ECR output, and image push remain gated by `apply=true`. The live migration/smoke path is VPC-reachable by default through ECS one-off tasks, and the server image now includes migration/smoke scripts plus migrations. The smoke runner requires an explicit network context (`ecs-run-task`, `bastion-vpn`, or `runner-security-group`) so ordinary host-side RDS/Redis smoke is not implied. Terraform `count` gates for ECS secret policies and Redis alarms now use plan-known inputs/names instead of apply-time generated ARNs/ids; Redis live resources set `transit_encryption_mode = "required"` to match the TLS-only contract; ECS API runtime now receives an avatar upload host, API IAM can sign `avatars/*` uploads, API/migration/smoke DB clients use `DATABASE_SSL=true` for live RDS, public API traffic redirects to HTTPS with an ACM cert requirement, and S3 receipt/avatar uploads get CORS from `ALLOWED_ORIGINS`; ECS now explicitly waits for IAM module policy attachments before task definitions/services. Static proof only; no live `plan`, `apply`, AWS resource, image push, RDS migration, Redis/S3 smoke, ECS deploy, or runtime proof yet; spreadsheet task 1.2 remains not done. |
...
- `TB-INFRA-001` 2026-07-09 review follow-up: ECS plan now runs for `create_ecs=true` during plan-only dispatch, while apply/image-push stays apply-gated. Live migration/smoke proof now defaults to ECS one-off tasks inside the VPC; host-side smoke requires an explicit bastion/VPN or runner-security-group context. Terraform `count` expressions for secret policies and Redis alarms avoid unknown-after-apply values, Redis enforces TLS-only mode, API/migration/smoke DB clients use `DATABASE_SSL=true` for live RDS, public API ingress redirects to HTTPS with an ACM cert requirement, S3 upload CORS uses `ALLOWED_ORIGINS`, and ECS waits for IAM module policy attachments before task definitions/services. Static checks were run only; no live AWS plan/apply, resource creation, ECS deploy, RDS migration, or runtime smoke proof is claimed.
