...
| TB-TRUST-001 | Restaurant `trust_score` is computed by backend rules from reviews, weighted by `trust_weight_bucket` and reviewer rank (Anti-Fraud §10) | yes | no | no | no | in_progress | 2026-07-10: pure `computeTrustScore`/`weightForReview` calculator + `recomputeRestaurantTrustScore` service delivered with unit proof (`tests/unit/trust/` 2 files / 19 tests; `npm run test:unit` 23 files / 251 tests; `server:build` 107 files). Decision 0018. No schema change. Deferred: wiring recompute into verification/admin/deletion triggers, and live DB recompute proof (local Docker unavailable). Earlier "implemented" row was corrected — no recompute service existed before this slice. |
