...
| TB-REVIEW-001 | Users can create verified food reviews tied to restaurant, receipt, and verification state | yes | yes | no | no | implemented | 2026-07-08 Phase 4 backend Review & OCR closeout: PR #36/TB-FRAUD-001 dependency verified merged into `main`; db:migrate applied 0; full server test passed 31 files / 1 skipped and 306 tests / 2 skipped; server:build passed 102 files; story verify passed. Covers 4.1 review create API, 4.2 receipt upload/private S3/idempotency/duplicate hash proof, 4.3 upload-to-OCR enqueue, 4.4 receipt verification lifecycle proof through PR #36, and 4.5 owner-scoped review status API. UI/mobile/admin UI excluded. |
