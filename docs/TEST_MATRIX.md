...
| TB-INFRA-001 | Real AWS MVP infrastructure baseline provisions non-production RDS/PostgreSQL+PostGIS, private S3 receipts, Redis/BullMQ cache, ECR, ECS API/worker runtime, IAM/secrets, CloudWatch, deploy workflow, smoke proof, and rollback notes | yes | no | no | yes | in_progress | 2026-07-09 PR #41 Greptile/CI follow-up: API task role no longer grants `s3:DeleteObject` on receipt evidence; live Redis AUTH now requires explicit Terraform/workflow plaintext-in-state approval before plan/apply; CI static verifier runs Terraform Docker containers as the Linux runner UID/GID with `HOME=/tmp` to avoid root-owned temp cleanup `EACCES`; `REDIS_TLS=1` now enables TLS and `.env.example` documents `DATABASE_SSL_REJECT_UNAUTHORIZED`. Static proof passed `actionlint`, `node --check`, targeted Redis config unit proof 1 file / 3 tests, and `npm run verify:tb-infra-static` with Terraform fmt/init/validate/no-live plan, `server:build` 106 files, server Docker image build, and whitespace checks. Static proof only; no live `plan`, `apply`, AWS resource, image push, RDS migration, Redis/S3 smoke, ECS deploy, or runtime proof yet; spreadsheet task 1.2 remains not done. |
...
- `TB-INFRA-001` 2026-07-09 PR #41 review follow-up: API receipt IAM is read/write only, Redis AUTH live plan/apply requires explicit plaintext-in-state approval, static verifier avoids Linux CI root-owned Docker temp files, `REDIS_TLS=1` is covered by unit proof, and `DATABASE_SSL_REJECT_UNAUTHORIZED` is documented. Static checks were run only; no live AWS plan/apply, resource creation, ECS deploy, RDS migration, or runtime smoke proof is claimed.
