...
| Mobile map | MapLibre/AWS Location map-first view with Vietnamese floating search, persistent four-item navigation, and a snapping nearby sheet tucked behind navigation when collapsed; consumes visible viewport bounds and backend-normalized place/route DTOs | Complete redesigned marker/card, place-selection, and route-line interaction smoke with nearby fixture data before release claim |
